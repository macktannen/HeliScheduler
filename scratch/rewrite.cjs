...
const eventModalPath = path.join(__dirname, '../src/components/EventModal.jsx');
let content = fs.readFileSync(eventModalPath, 'utf8');

const importRegex = /import \{ (.*?) \} from 'lucide-react';/;
content = content.replace(importRegex, "import { $1, Plane, MessageSquare, BookOpen, Clock } from 'lucide-react';");

const componentStart = content.indexOf('// --- EVENT MODAL ---');
const beforeComponent = content.substring(0, componentStart);

const newComponent = `// --- EVENT MODAL ---
...
fs.writeFileSync('scratch/rewrite-eventmodal.js', beforeComponent + newComponent);
